export default function autocompleteHelper (result, callback) {
  ...
  if (result.types.indexOf('street_address') >= 0) {
    ...
    postcodeObject = result.address_components.length - 1;
    cityObject = result.address_components.length - 4;
    ...
    postcodeObject = result.address_components.length - 1;
    cityObject = result.address_components[1];
    formattedName = result.name;
    ...
    const CountryObject = result.address_components.length - 1;
